...
export default function CashFlowDashboard() {
  ...
  const [cashFlowData, setCashFlowData] = useState<CashFlowDataProps[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // Transform department data into bank accounts
        const accounts = departments.map((dept, index) => ({
          id: index + 1,
          name: dept.mName,
          balance: Math.floor(Math.random() * 50000) + 10000, // Random balance between 10k-60k
          accountNumber: `**** ${Math.floor(1000 + Math.random() * 9000)}` // Random 4 digits
        }));
        setBankAccounts(accounts);
        
        // Transform department data into transactions
        const departmentTransactions = departments.slice(0, 20).map((dept, index) => ({
          id: index + 1,
          date: new Date(Date.now() - (index * 24 * 60 * 60 * 1000)).toISOString().split('T')[0],
          description: `Transaction - ${dept.mName}`,
          amount: Math.floor(Math.random() * 10000) + 1000,
          type: Math.random() > 0.5 ? "income" : "expense"
        }));
        setTransactions(departmentTransactions);
        
        // Set cash on hand based on department count
        setCashOnHand(departments.length * 1000);
        
        // Generate cash flow data based on departments
        const months = ["Jan", "Feb", "Mar", "Apr", "May", "Jun"];
        const flowData = months.map((month, i) => ({
          month,
          inflow: Math.floor((departments.length * 10000) * (0.8 + Math.random() * 0.4)),
          outflow: Math.floor((departments.length * 8000) * (0.8 + Math.random() * 0.4))
        }));
    ...
    return (
      <div className="w-full h-full bg-[#F0F0F0] p-6 overflow-auto font-sans flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-4 text-gray-700">Loading cash flow data...</p>
        </div>
      </div>
    );
  }
  ...
  // Calculate net cash flow for the current month
  const currentMonth = new Date().toLocaleString('default', { month: 'short' });
  const currentMonthData = cashFlowData.find(data => data.month === currentMonth) || { inflow: 0, outflow: 0 };
  const netCashFlow = currentMonthData.inflow - currentMonthData.outflow;
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
          ...
          <div className="bg-white rounded-lg shadow-md p-6 border-l-4 border-blue-600">
            ...
            <div className="text-sm text-gray-600 mt-2">
              {netCashFlow >= 0 ? '+' : ''}{((netCashFlow / (currentMonthData.outflow || 1)) * 100).toFixed(1)}% from last month
            </div>
          </div>
        </div>
