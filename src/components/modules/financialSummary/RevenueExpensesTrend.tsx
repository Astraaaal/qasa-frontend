...
import { useAuth } from "@/components/service/hooks/useAuth"; // Import the authentication hook
...
const RevenueExpensesTrend = ({
  ...
  const [data, setData] = useState<{ month: string; revenue: number; expenses: number; }[]>([]);
  ...
  useEffect(() => {
    const loadDepartments = async () => {
      ...
      try {
        ...
        const transformedData = response.map((department: any) => ({
          month: department.mName, // Assuming mName is the month
          revenue: department.mRevenue || 1000, // Assuming mRevenue is the revenue
          expenses: department.mExpenses || 2000, // Assuming mExpenses is the expenses
        }));
